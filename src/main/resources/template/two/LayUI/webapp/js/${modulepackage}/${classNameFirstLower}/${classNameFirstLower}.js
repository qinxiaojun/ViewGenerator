...
className = table.className >
    < #assign
classNameLower = className ? uncap_first >
/* ------------------------------  LayUI已初始化  ------------------------------ */
/* ------------------------------  基础初始化 Start  ------------------------------ */
// 数据表格绑定
var ${classNameLower}Table = table.render({
        ...
        cols: [[
            ...
            < #if(column.columnNameLower == 'invalidFlag') >
            {
                ...
                title: ${column.columnNameLower}Desc,
                align: 'center',
                sort: true,
    ...
    toolbarSettings: {
        ...
        toolbarExtraButtons: [
            $('<button id="prevBtn"></button>').text('上一步').addClass('layui-btn layui-btn-normal')
                .on('click', function () {
                    $('#${classNameLower}SmartWizard').smartWizard("prev");
                }),
            $('<button id="nextBtn"></button>').text('下一步').addClass('layui-btn layui-btn-normal')
                .on('click', function () {
                    $('#${classNameLower}SmartWizard').smartWizard("next");
                }),
...
function reloadTable() {
    ${classNameLower}
    Table = table.reload('${classNameLower}Table', {
        page: {
            // 重新定位当前页
...
:
    $.trim($("#search${column.columnName}").val()) < #if
    column_has_next >,
<
    /#if>
    < /#list>
...
function getData() {
    ...
    "${column.columnNameLower}"
:
    $.trim($("#${column.columnNameLower}").val()) < #if
    column_has_next >,
<
    /#if>
    < /#list>
...
function writeData(data) {
    ...
    axios.post(contextPath + '${classNameLower}/prepareUpdate', {
        < #list
    table.compositeIdColumns as column >
    '${column.columnNameLower}'
:
    data.$
    {
...
<
    ...
    then(function (response) {
        ...
        <
            #list
            table.columns as column >
            $('#${column.columnNameLower}').val(${classNameLower}Dto.$
            {
                column.columnNameLower
            }
        )
