...
import com.example.common.GeneratorVO;
...
public class DataBaseController {
    ...
    public ResponseBean genTable(@PathVariable("tableName") String tableName, GeneratorVO generatorVO) throws IOException {
        // 配置表名
        ...
        if (genCode(tableNames, generatorVO, outRoot)) {
    ...
    public void genTableToZip(@PathVariable("tableName") String tableName, GeneratorVO generatorVO,
        ...
        // 生成代码到临时路径
        if (genCode(tableNames, generatorVO, tempDir.getPath())) {
            StringBuilder comment = new StringBuilder("mioto company code generator\r\n\r\n");
                    ...
                    if (Boolean.parseBoolean(config.get("isRead"))) {
                        ...
                        while (entries.hasNext()) {
                            ...
                            if (Constant.TEMPLATE.equals(entry.getKey())) {
                                // 查看配置路径下macro.include文件是否存在
                                File file = new File(ResourceUtils.getURL("classpath:").getPath() + entry.getValue() + "/macro.include");
                                if (!file.exists()) {
                                    // 模板代码位置不存在
     ...
     * @throws IOException
     */
    public boolean genCode(String[] tableNames, GeneratorVO generatorVO, String outRoot) throws IOException {
        ...
        generator.addTemplateRootDir(ResourceUtils.getURL("classpath:").getPath() + template);
        // 开始执行
        try {
            for (String tableName : tableNames) {
                DisplayField.setDisplayField(tableName, generatorVO.getIgnoreProp());
                SearchField.setSearchField(tableName, generatorVO.getSearchProp());
                FormField.setFormField(tableName, generatorVO.getFormProp());
                SortField.setSortField(tableName, generatorVO.getSortProp());
                //设置表名注释
                Map<String,String> map = generatorDao.queryTable(tableName);
                GeneratorProperties.setProperty("tableComment",map.get("tableComment"));
                // 删除旧文件
            ...
            log.error("----- 生成失败 请检查数据库是否连接正常及表名是否正确以及权限是否缺失 -----");
            e.printStackTrace();
            return false;
        }
        return true;
